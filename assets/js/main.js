...
function updateHTMl(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        memoContREG += `<br/>`
        for (let index = 0; index < tagText.length; index++) {
            const element = tagText[index];
            var noElement = element.replace('#', "")
            memoContREG += `<span class='tagged-text'><a rel='noopener noreferrer' href='${element}'>${noElement}</a></span> `;
        }
